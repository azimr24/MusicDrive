...
export const MusicPlayerProvider = ({children}) => {
    ...
    const [gLastQSongIndex, setLastQSongIndex] = useState(null);
    const [gIsPlaying, setGIsPlaying] = useState(false);


    const isProcessing = useAnimatedRef(false)
    ...
    const handlePrevSong = async () => {
        ...
        try {
            ...
            if (isLoaded) {
                ...
                setGCurrSongIndex((currSongIndex) => (currSongIndex - 1 + numSongs) % numSongs)
                currSong.current = song
                setTitle(() => title)
                console.log(title)
                setDuration(() => durationMillis)
    ...
    const shuffleSongs = (songs) => {
        if (!shuffle) {
            ...
            const currSongIndex = songsBaseOrder.findIndex(({ song }) => song === currSong.current)
    ...
    return (
       <MusicPlayerContext.Provider value={{
            ...
            loading, setLoading,
            stopSong,
            playlistArtistName, setPlaylistArtistName,
            // gLastQSongIndex, setLastQSongIndex,
            songsBaseOrder, setSongsBaseOrder,
            shuffleSongs, repeatSongs,
            unloadSongs
